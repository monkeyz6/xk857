module.exports = [
  {text: '首页', link: '/', icon: 'reco-home'},
  {
    text: 'Java', icon: 'iconfont icon-JavaCorefenxigongju', items: [
      {
        text: 'java开发技巧',
        icon: 'iconfont icon-java',
        link: '/后端开发/java/java高效编程常用技术优化手段/资源关闭优化方案'
      }, {
        text: '消息队列专栏',
        icon: 'iconfont icon-xiaoxiduilie',
        link: '/后端开发/中间件/RabbitMQ/主流的消息队列和选择问题'
      }, {
        text: '第三方支付',
        icon: 'iconfont icon-weixinzhifu',
        link: '/后端开发/第三方支付/微信支付V3/微信支付准备工作'
      }, {
        text: 'SpringBoot',
        icon: 'iconfont icon-Leaf',
        link: '/Spring专栏/SpringBoot使用技巧/面向切面编程AOP'
      },
      {
        text: 'SpringCloud',
        icon: 'iconfont icon-mseweifuwuyinqing',
        link: '/微服务/SpringCloudAlibaba/分布式架构理论'
      },
    ]
  },
  {
    text: '前端开发', icon: 'iconfont icon-codepen-circle-fill', items: [
      {
        text: 'Vue框架',
        icon: 'iconfont icon-Vue',
        link: '/前端开发/vue/Vue3核心开发手册/使用Vite创建vue3项目'
      },
      // {
      //   text: '原生js',
      //   icon: 'iconfont icon-js',
      //   link: '/前端开发/js/CSS设计模式'
      // },
      {
        text: '基础扩展',
        icon: 'iconfont icon-sheji-xianxing',
        link: '/前端开发/前端基础/css/CSS设计模式'
      },
      {
        text: '插件开发',
        icon: 'iconfont icon-menu_cjgl',
        link: '/插件开发/IDEA插件开发常用API/插件开发之创建菜单'
      },
    ]
  },
  {text: '数据库', icon: 'iconfont icon-shujuku1', link: '/数据库/新零售行业数据库设计/'},
  {text: 'Redis专栏', icon: 'iconfont icon-Redis', link: '/后端开发/redis专栏/高阶使用技巧/Redis概述及基本使用'},
  {text: '常用框架', icon: 'iconfont icon-zuzhijiagou', link: '/后端开发/常用框架/MybatisPlus/SpringBoot整合MybatisPlus'},

  {text: 'Linux', icon: 'iconfont icon-centos', link: '/Linux/Centos7/Linux常用命令'},
  {text: '关于', icon: 'iconfont icon-f-about', link: '/about/'},
  {text: '时间轴', link: '/timeline/', icon: 'reco-date'}
]
